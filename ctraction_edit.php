...
//	6/26/2013	added an if statement that pulls either nda/license_action based on $module
...
//ctraction_edit.php -- User Access Level: User
include("accesscontrol.php");
html_header($fullname,$module,$orgadmin,$user_level);
$sql="SELECT title, docket_alt FROM contracts WHERE ID='$CTR_ID'";
...
if ($I=="1"){
$sql="SELECT * FROM ctr_actions WHERE action_ID='$ACTION_ID'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
    $CTR_ID = $row["ctr_ID"];
...
<!-- ADD OR EDIT Contract ACTIONS -->
<? if ($EDIT=="Y" and ($sysadmin=="Y" or $user_level != "Viewer")){?>
<table align="right" border="0" cellpadding="0" cellspacing="0"><tr>
  <td width="100%" align="center" bgcolor="#FFFFFF">
    <? if ($CTR_ID!="0" and $ACTION_ID!="0"){?>
    <a href="delete_confirm.php?module=<?=$module;?>&TABLE=ctr_actions&ID=<?=$ACTION_ID;?>&NAME=Action For Docket No. <?=$docket_alt;?>">Delete</a>&nbsp;|&nbsp;
    <?}?>
...
<table align="center" width="100%" border="0" cellpadding="0" cellspacing="10">
  ...
  <tr>
      ...
      <td width="400">		  			
		  ...
		  <?
		  if ($module=="NDAs") {$sql="SELECT * FROM menus WHERE menu_type='NDA_ACTION' ORDER BY menu_name";}
		  else {$sql="SELECT * FROM menus WHERE menu_type='LICENSE_ACTION' ORDER BY menu_name";}
		  $result=mysql_query($sql);
...
if ($ACTION_ID=="0" or $duplicate=="Y") { // ADD NEW RECORD
  $sql="INSERT INTO ctr_actions SET
...
<p> To view disclosure records, click <a href="ndas_list.php?module=<?=$module;?>&SORT=ALL">here for NDAs</a>, <a href="licenses_list.php?module=<?=$module;?>&SORT=ALL">here for Licenses.</a></p>
</td></tr></table><br>
<?}}?>
<!-- READ -->
<? if ($EDIT=="N"){?>
<table align="right" border="0" cellpadding="0" cellspacing="0"><tr>
  <td width="100%" align="center" bgcolor="#FFFFFF">
    ...
    <a href="<?=$PHP_SELF;?>?module=<?=$module;?>&CTR_ID=<?=$CTR_ID;?>&ACTION_ID=<?=$ACTION_ID;?>&I=1&EDIT=Y">Edit</a>&nbsp;|&nbsp;
    <?}?>
    <a href="JavaScript:window.close()">Close</a>
  </td></tr>
</table><br><br>

...
//  	6/25/2013   	created for uploading genIPM docs. Created using discdoc_upload.php
//              	updated to work with genIPM
...
<?
$sql="SELECT title, docket FROM genipm_filings WHERE ID='$GENIPM_ID'";
...
<!-- ADD OR EDIT General IPM Documents -->
<? if ($EDIT=="Y" and ($sysadmin=="Y" or $user_level != "Viewer")){?>
    <table align="right" border="0" cellpadding="0" cellspacing="0"><tr>
      <td width="100%" align="center" bgcolor="#FFFFFF">
        <? if ($GENIPM_ID!="0" and $id!="0"){?>
        <a href="delete_confirm.php?module=<?=$module;?>&TABLE=genipm_documents&ID=<?=$id;?>&NAME=Document For Docket No. <?=$docket;?>&alt_name=<?=$alt_name;?>">Delete</a>&nbsp;|&nbsp;
        <?}?>
	...
	$alt_name = "g" . "$GENIPM_ID" . "_" . $_FILES["file"]["name"];

	//if (file_exists("\\AppsMyDocket:Apps1937@korry.com\kdata\korry_data\AppData\MyDocket\\" . $alt_name))
	//if (file_exists("\\\\kdata\\korry_data\\AppData\\MyDocket\\" . $alt_name))
	if (file_exists($absolute_documents . $alt_name))
	    {
		echo "We're sorry, but a file by that name already exists under this General IPM.";
		?>&nbsp;&nbsp;<a href="cpydoc_upload.php?module=<?=$module;?>&GENIPM_ID=<?=$GENIPM_ID;?>&id=0&I=0&EDIT=Y">Try Again</a><?
		html_footer();
		exit;
	    }
	else
	...
	$alt_name = mysql_real_escape_string($alt_name);
      ...
      $sql="INSERT INTO genipm_documents SET
        genipm_ID='$GENIPM_ID',
        subtype='$subtype',
        name='$name',
	doc_date='$doc_date',
	...
	<?}}
	    ...
	    $sql = "UPDATE genipm_documents SET doc_date='$doc_date', subtype='$subtype' WHERE id=$id";
	    if(!mysql_query($sql)){
		echo mysql_error();
		exit;
	    }

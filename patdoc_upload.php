...
//  	6/24/2013   	created for uploading patent docs. Created using discdoc_upload.php
//              	updated to work with patents
...
//	8/2/2013	deleted max file size; changed in php.ini to 50M; added error msg when it's too big
//			added docket_date
//	8/9/2013	allowed special chars in date for durability
//	8/15/2013	checked date
...
//patdoc_upload.php -- 	User Access Level: User/Viewer
...
<!-- ADD OR EDIT patent Documents -->
<? if ($EDIT=="Y" and ($sysadmin=="Y" or $user_level != "Viewer")){?>
    <table align="right" border="0" cellpadding="0" cellspacing="0"><tr>
      <td width="100%" align="center" bgcolor="#FFFFFF">
        <? if ($PAT_ID!="0" and $id!="0"){?>
        <a href="delete_confirm.php?module=<?=$module;?>&TABLE=pat_documents&ID=<?=$id;?>&NAME=Document For Docket No. <?=$docket;?>&alt_name=<?=$alt_name;?>">Delete</a>&nbsp;|&nbsp;
        <?}?>
	...
	$alt_name = "p" . "$PAT_ID" . "_" . $_FILES["file"]["name"];

	if (file_exists($absolute_documents . $alt_name))
	    {
		echo "We're sorry, but a file by that name already exists under this patent.";
		?>&nbsp;&nbsp;<a href="patdoc_upload.php?module=<?=$module;?>&PAT_ID=<?=$PAT_ID;?>&id=0&I=0&EDIT=Y">Try Again</a><?
		html_footer();
		exit;
	    }
	else
	...
	$alt_name = mysql_real_escape_string($alt_name);
      $sql="INSERT INTO pat_documents SET
        pat_ID='$PAT_ID',
        subtype='$subtype',
	doc_date='$doc_date',
        name='$name',
	...
	<?}}
	    ...
	    $sql = "UPDATE pat_documents SET doc_date='$doc_date', subtype='$subtype' WHERE id=$id";
	    if(!mysql_query($sql)){
		echo mysql_error();
		exit;
	    }

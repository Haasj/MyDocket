...
//	6/24/2013	created for uploading contracts docs. Created using discdoc_upload.php
//			updated to work with contracts
//			Note: this should work for both NDAs and Licenses, since they are both contracts so they have different ID numbers. there will be no overlap.
//	6/26/2013	made it so there could be different types of docs for NDAs/Licensens with the if statement for the module
...
//	8/2/2013	deleted max file size; changed in php.ini to 50M; added error message when it's too big
//			added doc_date
//	8/9/2013	allowed special chars in date for durability
//	8/15/2013	checked the date
//			allowed editing. I had to make the add and edit sceens two different ones becasue they had different form types. I'm not allowing them to change the file itself, jsut the type and date
...
//ctrdoc_upload.php -- 	User Access Level: User
include("accesscontrol.php");
html_header($fullname,$module,$orgadmin,$user_level);
$sql="SELECT title, docket FROM contracts WHERE ID='$CTR_ID'";
...
<? if ($EDIT=="Y" and ($sysadmin=="Y" or $user_level != "Viewer")){?>
    <table align="right" border="0" cellpadding="0" cellspacing="0"><tr>
      <td width="100%" align="center" bgcolor="#FFFFFF">
        <? if ($CTR_ID!="0" and $id!="0"){?>
        <a href="delete_confirm.php?module=<?=$module;?>&TABLE=ctr_documents&ID=<?=$id;?>&NAME=Document For Docket No. <?=$docket;?>&alt_name=<?=$alt_name;?>">Delete</a>&nbsp;|&nbsp;
        <?}?>
    ...
    if ($upload==""){?>
        ...
        <form method=post action="<?=$PHP_SELF;?>" enctype="multipart/form-data">
	          <input type="hidden" name="CTR_ID" value="<?=$CTR_ID;?>">
        ...
        <table align="center" width="100%" border="0" cellpadding="0" cellspacing="10">
          <tr>
			  ...
			  else {$sql="SELECT * FROM menus WHERE menu_type='LICENSE_DOCUMENT' ORDER BY menu_name";}
                          $result=mysql_query($sql);
	...
	$alt_name = "ct" . "$CTR_ID" . "_" . $_FILES["file"]["name"];
	
	
	
	if (file_exists($absolute_documents . $alt_name))
	    {
		echo "We're sorry, but a file by that name already exists under this contract.";
		?>&nbsp;&nbsp;<a href="ctrdoc_upload.php?module=<?=$module;?>&CTR_ID=<?=$CTR_ID;?>&id=0&I=0&EDIT=Y">Try Again</a><?
		html_footer();
		exit;
	    }
	else
	...
	$alt_name = mysql_real_escape_string($alt_name);
      ...
      $sql="INSERT INTO ctr_documents SET
        ctr_ID='$CTR_ID',
        subtype='$subtype',
        name='$name',
	alt_name='$alt_name',
	...
	<?}}
	    ...
	    $sql = "UPDATE ctr_documents SET doc_date='$doc_date', subtype='$subtype' WHERE id=$id";
	    if(!mysql_query($sql)){
		echo mysql_error();
		exit;
	    }

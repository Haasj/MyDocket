...
//	6/12/2013	created using pataction_edit.php
//			replaced all pat_filings->disc_filings, pat_actions->disc_actions, replaced all pat with disc
//			updated menus db to reflect the same action types as tms and patents.
//			This should be done. if it doesn't work, it's most likely not a problem with this
//	6/17/2013	changed a broken link
//	6/26/2013	updated to use docket_alt instead of docket
...
// discaction_edit.php -- User Access Level: User
include("accesscontrol.php");
html_header($fullname,$module,$orgadmin,$user_level);
$sql="SELECT title, docket_alt FROM disc_filings WHERE disc_ID='$DISC_ID'";
...
if ($I=="1"){
$sql="SELECT * FROM disc_actions WHERE action_ID='$ACTION_ID'";
$result=mysql_query($sql);
$row=mysql_fetch_array($result);
    $DISC_ID = $row["disc_ID"];
...
<!-- ADD OR EDIT Disclosure ACTIONS -->
<? if ($EDIT=="Y" and ($sysadmin=="Y" or $user_level != "Viewer")){?>
<table align="right" border="0" cellpadding="0" cellspacing="0"><tr>
  <td width="100%" align="center" bgcolor="#FFFFFF">
    <? if ($DISC_ID!="0" and $ACTION_ID!="0"){?>
    <a href="delete_confirm.php?module=<?=$module;?>&TABLE=disc_actions&ID=<?=$ACTION_ID;?>&NAME=Action For Docket No. <?=$docket_alt;?>">Delete</a>&nbsp;|&nbsp;
    <?}?>
...
if ($ACTION_ID=="0" or $duplicate=="Y") { // ADD NEW RECORD
  $sql="INSERT INTO disc_actions SET
...
<p> To view disclosure records, click <a href="disclosures_list.php?module=<?=$module;?>&SORT=ALL">here</a></p>
<p> To add another record, click <a href="disclosures_edit.php?module=<?=$module;?>&DISC_ID=<?=$DISC_ID;?>&ACTION_ID=0&I=0&EDIT=Y">here</a></p>
</td></tr></table><br>
<?}}?>
<!-- READ -->
<? if ($EDIT=="N"){?>
<table align="right" border="0" cellpadding="0" cellspacing="0"><tr>
  <td width="100%" align="center" bgcolor="#FFFFFF">
    ...
    <a href="<?=$PHP_SELF;?>?module=<?=$module;?>&DISC_ID=<?=$DISC_ID;?>&ACTION_ID=<?=$ACTION_ID;?>&I=1&EDIT=Y">Edit</a>&nbsp;|&nbsp;
    <?}?>
    <a href="JavaScript:window.close()">Close</a>
  </td></tr>
</table><br><br>

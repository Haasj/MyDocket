...
//  	6/17/2013   	created this for uploading docs. I couldn't figure it out a simpler way, so it'll be like the actions. this will also allow for easier multiple uploads
//              	created using discaction_edit
//              	$ACTION_ID->id, disc_actions->disc_documents, action_type->subtype
//              	getting the same issue. I think it is because there are two posts in the same file, which is causing problems.
//              	I think I can correct this if I put them both in the same form; the hidden inputs at the front don't seem to care if it is formdata or not
//              	another issue is the check to see if stuff is done. when one thing is done, it deletes the other one, and vice versa. I'm getting rid of this error with upload/ok by making the upload
//			button essentially the ok button. with regards to the other field (doc type), I'm deleting the check for now and hoping that it will continue to provide data
//  	6/18/2013	got it working! ptl
//			I don't know exactly why it is working now; i changed it back to two forms (which I changed yesterday after hours of trying to get it to work) and it suddenly worked when I was editing
//			something else
//			saved a version on the destop that works. haven't tried with editing yet, but that shouldn't be an issue since you'll only ever need to delete the document.
//			ya the update doesn't work. I'm going to delete the functionality. When you click on the document it will just show the record with the option to delete it. I will also attempt to make
//			a delte on update
//			disabled edit functionality.
//			created alt_name in table. this way, it displays the name and everything looks right with the name, but the file is saved as a different name so you can have the same file names.
//			Note: the alt_name is just the name with the disc_id on it, so you still can't have duplicate file names attached to the same disclosure
//			everything works fine here. now the only thing left to do (if I can) is to delete the file when you delete it from the app. this will be done in delete_confirm.php
//			that's done. also, changed the alt_name to be d"disc_id"_"file name". d for disclosures
//  	6/24/2013	added try again functionality if the name of the doc already exists
...
//	7/30/2013	updated to use session variables instead of full paths
//	8/2/2013	deleted max file size and changed it in php.ini to 50M
//	8/9/2013	allowed special chars in date for durability
...
// discdoc_upload.php -- User Access Level: User
include("accesscontrol.php");
html_header($fullname,$module,$orgadmin,$user_level);
$sql="SELECT title, docket FROM disc_filings WHERE disc_ID='$DISC_ID'";
...
<!-- ADD OR EDIT Disclosure Documents -->
<? if ($EDIT=="Y" and ($sysadmin=="Y" or $user_level != "Viewer")){?>
    <table align="right" border="0" cellpadding="0" cellspacing="0"><tr>
      <td width="100%" align="center" bgcolor="#FFFFFF">
        <? if ($DISC_ID!="0" and $id!="0"){?>
        <a href="delete_confirm.php?module=<?=$module;?>&TABLE=disc_documents&ID=<?=$id;?>&NAME=Document For Docket No. <?=$docket;?>&alt_name=<?=$alt_name;?>">Delete</a>&nbsp;|&nbsp;
        <?}?>
        <a href="JavaScript:window.close()">Close</a>
      </td></tr>
    </table><br><br>
    <?if ($id=="0"){
	echo("<center>ADD Document</center>");
	if ($upload==""){?>
    ...
    if($submit == ""){	// ADD NEW RECORD
        $name = $_FILES["file"]["name"];
	$alt_name = "d" . "$DISC_ID" . "_" . $_FILES["file"]["name"];

	if (file_exists($absolute_documents . $alt_name))
	    {
		echo "We're sorry, but a file by that name already exists under this disclosure.";
		?>&nbsp;&nbsp;<a href="discdoc_upload.php?module=<?=$module;?>&DISC_ID=<?=$DISC_ID;?>&id=0&I=0&EDIT=Y">Try Again</a><?
		html_footer();
		exit;
	    }
	else
	...
	$alt_name = mysql_real_escape_string($alt_name);
      ...
      $sql="INSERT INTO disc_documents SET
        disc_ID='$DISC_ID',
        subtype='$subtype',
        name='$name',
	alt_name='$alt_name',
	...
	<?}}
	    ...
	    $sql = "UPDATE disc_documents SET doc_date='$doc_date', subtype='$subtype' WHERE id=$id";
	    if(!mysql_query($sql)){
		echo mysql_error();
		exit;
	    }

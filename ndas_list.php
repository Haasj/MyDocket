...
//			added docket functionality from disclosures_list. Note: doesn't work
//	6/26/2013	made it work. Added expiration date column at the far right.
//	7/3/2013	added client on list A
//	7/10/2013	deleted list label on the top
//			added secondary sort functionality. for details, see patent_list.php
//	7/17/2013	added $user_level to header call
//	7/19/2013	enabled index sort for searches. there wasn't a specialized query
//	7/23/2013	added parties/ecs poc to the var string at the top and search queries
//	7/30/2013	enabled custom number listings. for more details see patent_list.php
//	8/1/2013	enabled custom listings. for more details see patents_list.php
//	8/2/2013	added link on title to record from custom listing
...
elseif ($SORT=="SEARCH")
  ...
  "effective_date=$effective_date&expiry_date=$expiry_date&ecs_poc=$ecs_poc&parties_poc=$parties_poc"; 
else $var="SORT=ALL";
// Set Defaults
if($number=="") {$number=50;}
...
<input type="hidden" name="ecs_poc" value="<?=$ecs_poc;?>">
<input type="hidden" name="parties_poc" value="<?=$parties_poc;?>">
...
<?
if ($SORT=="ALL") echo("<center>LIST ALL NON-DISCLOSURE AGREEMENTS");
else echo("<center>LIST NON-DISCLOSURE AGREEMENT SEARCH RESULTS");?><br><br>
<br><?}?>
...
<tr bgcolor=EEEEEE><font size="-2">
  ...
  <td width="125"><U>Direction</U>
    <br><a href="<?=$PHP_SELF;?>?module=<?=$module;?>&SORT=<?=$SORT;?>&DIRECTION=<?=$DIRECTION;?>&ORDER=direction&<?=$var?>&ORDER2=<?=$ORDER2;?>"><img src="up.gif" width="13" height="11" alt="" border="0" title="Primary"></a>
    <a href="<?=$PHP_SELF;?>?module=<?=$module;?>&SORT=<?=$SORT;?>&DIRECTION=<?=$DIRECTION;?>&ORDER=direction DESC&<?=$var?>&ORDER2=<?=$ORDER2;?>"><img src="down.gif" width="13" height="11" alt="" border="0" title="Primary"></a>
    <a href="<?=$PHP_SELF;?>?module=<?=$module;?>&SORT=<?=$SORT;?>&DIRECTION=<?=$DIRECTION;?>&ORDER=<?=$ORDER;?>&<?=$var?>&ORDER2=direction"><img src="up2.gif" width="13" height="11" alt="" border="0" title="Secondary"></a>
    <a href="<?=$PHP_SELF;?>?module=<?=$module;?>&SORT=<?=$SORT;?>&DIRECTION=<?=$DIRECTION;?>&ORDER=<?=$ORDER;?>&<?=$var?>&ORDER2=direction DESC"><img src="down2.gif" width="13" height="11" alt="" border="0" title="Secondary"></a></td>
</font></tr>
<?
// SQL Query
if ($SORT == "ALL")
    $sql="SELECT * FROM contracts WHERE customer_ID = '$customer_ID' and contract_type='NDA' ORDER BY $ORDER, $ORDER2, docket_alt LIMIT $START, $number";
else // ($SORT == "SEARCH")
  $sql="SELECT * FROM contracts WHERE	  	  
    ...
    ecs_poc LIKE '%$ecs_poc%' and
    parties_poc LIKE '%$parties_poc%' and
...
if($ORDER2=="") {$ORDER2="docket_alt";}
 ...
 <br> <center>SELECT FIELDS</center><br><br>
  ...
  <tr bgcolor=EEEEEE>
    ...
    <td width="200"><input type="checkbox" name="check_nda_ID" value="Y">&nbsp;&nbsp;<U>NDA_ID</U></tr>
  <tr bgcolor=EEEEEE>
  ...
  <tr bgcolor=EEEEEE>  
    <td width="200"><input type="checkbox" name="check_ecs_poc" value="Y">&nbsp;&nbsp;<U>ECS Point of Contact</U>
    <td width="200"><input type="checkbox" name="check_parties_poc" value="Y">&nbsp;&nbsp;<U>Parties Point of Contact</U>
...
{?>
  ...
  $var=$var."&check_ecs_docket=$check_ecs_docket&check_firm_docket=$check_firm_docket&check_notes=$check_notes&".
      ...
      "check_firm_contact=$check_firm_contact&check_client=$check_client&check_nda_ID=$check_nda_ID&".
      "check_parties=$check_parties&check_direction=$check_direction&check_effective_date=$check_effective_date&".
      "check_expiry_date=$check_expiry_date&check_ecs_poc=$check_ecs_poc&check_parties_poc=$check_parties_poc&check_description=$check_description&".
	...
	<input type="hidden" name="check_nda_ID" value="<?=$check_nda_ID;?>">
	...
	<input type="hidden" name="check_ecs_poc" value="<?=$check_ecs_poc;?>">
	<input type="hidden" name="check_parties_poc" value="<?=$check_parties_poc;?>">
...
<center>CUSTOM NDA LISTING</center><br><br>
  <table align="center" width="100%" cellpadding="5">
  <tr bgcolor=EEEEEE><font size="-2">
<?
  ...
  <?if ($check_ecs_poc == "Y"){?>
  <td width="150"><U>ECS Point of Contact</U>
  <br><a href="<?=$PHP_SELF;?>?module=<?=$module;?>&REPORT=<?=$REPORT;?>&ORDER=ecs_poc&<?=$var?>&ORDER2=<?=$ORDER2;?>"><img src="up.gif" width="13" height="11" alt="" border="0" title="Primary"></a>
    <a href="<?=$PHP_SELF;?>?module=<?=$module;?>&REPORT=<?=$REPORT;?>&ORDER=ecs_poc DESC&<?=$var?>&ORDER2=<?=$ORDER2;?>"><img src="down.gif" width="13" height="11" alt="" border="0" title="Primary"></a>
    <a href="<?=$PHP_SELF;?>?module=<?=$module;?>&REPORT=<?=$REPORT;?>&ORDER=<?=$ORDER;?>&<?=$var?>&ORDER2=ecs_poc"><img src="up2.gif" width="13" height="11" alt="" border="0" title="Secondary"></a>
    <a href="<?=$PHP_SELF;?>?module=<?=$module;?>&REPORT=<?=$REPORT;?>&ORDER=<?=$ORDER;?>&<?=$var?>&ORDER2=ecs_poc DESC"><img src="down2.gif" width="13" height="11" alt="" border="0" title="Secondary"></a></td><?}?>
  <?if ($check_parties_poc == "Y"){?>
  <td width="150"><U>Parties Point of Contact</U>
  <br><a href="<?=$PHP_SELF;?>?module=<?=$module;?>&REPORT=<?=$REPORT;?>&ORDER=parties_poc&<?=$var?>&ORDER2=<?=$ORDER2;?>"><img src="up.gif" width="13" height="11" alt="" border="0" title="Primary"></a>
    <a href="<?=$PHP_SELF;?>?module=<?=$module;?>&REPORT=<?=$REPORT;?>&ORDER=parties_poc DESC&<?=$var?>&ORDER2=<?=$ORDER2;?>"><img src="down.gif" width="13" height="11" alt="" border="0" title="Primary"></a>
    <a href="<?=$PHP_SELF;?>?module=<?=$module;?>&REPORT=<?=$REPORT;?>&ORDER=<?=$ORDER;?>&<?=$var?>&ORDER2=parties_poc"><img src="up2.gif" width="13" height="11" alt="" border="0" title="Secondary"></a>
    <a href="<?=$PHP_SELF;?>?module=<?=$module;?>&REPORT=<?=$REPORT;?>&ORDER=<?=$ORDER;?>&<?=$var?>&ORDER2=parties_poc DESC"><img src="down2.gif" width="13" height="11" alt="" border="0" title="Secondary"></a></td><?}?>
...
while($row=mysql_fetch_array($result)){
  ...
  if ($check_title == "Y") {?>
  <td width="150" align="left"><small><a href="ndas_edit.php?module=<?=$module;?>&SORT=<?=$SORT?>&VAR=<?=$var?>&ID=<?=$ID?>&I=1&EDIT=N"><?=$title;?></a></small></td><?}
  if ($check_status == "Y") {?>
  ...
  if ($check_nda_ID == "Y") {?>
  <td width="150" align="left"><small><?=$ID;?></small></td><?}
  if ($check_parties == "Y") {?>
  ...
  if ($check_ecs_poc == "Y") {?>
  <td width="150" align="left"><small><?=$ecs_poc;?></small></td><?}
  if ($check_parties_poc == "Y") {?>
  <td width="150" align="left"><small><?=$parties_poc;?></small></td><?}
  if ($check_description == "Y") {?>
...
if($SORT == "ALL")
  ...
  $sql="SELECT * FROM contracts, ctr_actions WHERE
    ...
    contracts.parties LIKE '%$parties%' and
    contracts.ecs_poc LIKE '%$ecs_poc%' and
    contracts.parties_poc LIKE '%$parties_poc%' and
...
while($row=mysql_fetch_array($result)){ //on the ID number, on disc it pulls the same variable name from both tables. I grabbed this one from filings
  ...
  if($row["contract_type"] != "NDA")		//need this because for some reason it won't let me check for equality in the sql. this skips the rest of the loop if it's not an NDA
    continue;
...
//SQL Query for Selecting All $TYPE IP Records
if ($SORT == "ALL")
    $sql="SELECT * FROM contracts WHERE customer_ID = '$customer_ID' and contract_type='NDA' ORDER BY $ORDER, docket_alt LIMIT $START, $number";
else // ($SORT == "SEARCH")
  $sql="SELECT * FROM contracts WHERE	  	  
    customer_ID = '$customer_ID' and
    docket_alt LIKE '%$docket_alt%' and
...
?>
<tr bgcolor=EEEEEE>
  <td width="150"><small><a href="ndas_edit.php?module=<?=$module;?>&SORT=<?=$SORT?>&VAR=<?=$var?>&ID=<?=$ID?>&I=1&EDIT=N"><?=$ID;?></a></small></td>

...
//  	6/24/2013   	created for uploading trademark docs. Created using discdoc_upload.php
//              	updated to work with trademarks
...
//	8/2/2013	deleted max file path; changed in php.ini to 50M; added error msg when it's too big
//			added doc_date
...
//tmdoc_upload.php -- User Access Level: User/ Viewer
...
<? if ($EDIT=="Y" and ($sysadmin=="Y" or $user_level != "Viewer")){?>
    <table align="right" border="0" cellpadding="0" cellspacing="0"><tr>
      <td width="100%" align="center" bgcolor="#FFFFFF">
        <? if ($TM_ID!="0" and $id!="0"){?>
        <a href="delete_confirm.php?module=<?=$module;?>&TABLE=tm_documents&ID=<?=$id;?>&NAME=Document For Docket No. <?=$docket;?>&alt_name=<?=$alt_name;?>">Delete</a>&nbsp;|&nbsp;
        <?}?>
	...
	$alt_name = "t" . "$TM_ID" . "_" . $_FILES["file"]["name"];

	if (file_exists($absolute_documents . $alt_name))
	    {
		echo "We're sorry, but a file by that name already exists under this trademark.";
		?>&nbsp;&nbsp;<a href="tmdoc_upload.php?module=<?=$module;?>&TM_ID=<?=$TM_ID;?>&id=0&I=0&EDIT=Y">Try Again</a><?
		html_footer();
		exit;
	    }
	else
	...
	$alt_name = mysql_real_escape_string($alt_name);
      ...
      $sql="INSERT INTO tm_documents SET
        tm_ID='$TM_ID',
        subtype='$subtype',
        name='$name',
	alt_name='$alt_name',
	...
	<?}}
	    ...
	    $sql = "UPDATE tm_documents SET doc_date='$doc_date', subtype='$subtype' WHERE id=$id";
	    if(!mysql_query($sql)){
		echo mysql_error();
		exit;
	    }
